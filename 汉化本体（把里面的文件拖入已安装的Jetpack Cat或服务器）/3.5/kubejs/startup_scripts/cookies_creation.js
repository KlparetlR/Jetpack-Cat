	...
	event.create('gun_crate_rare').food(food => {
		food
          ...
          ctx.server.scheduleInTicks(2, ctx.server, function (callback) {
    	        callback.data.runCommand('/ftbquests change_progress ' + ctx.player.name + ' complete 1C87D9E0931E21CE');
    			})
      	})
	...
	event.create('gun_crate_super_rare').food(food => {
		food
    		...
    		.eaten(ctx => {
      		ctx.player.tell("嚼嚼！（咀嚼的声音）美味的木头！嗯！")
          ctx.server.scheduleInTicks(2, ctx.server, function (callback) {
    	        callback.data.runCommand('/ftbquests change_progress ' + ctx.player.name + ' complete 15F58CF7DEBCCD92');
    			})
      	})
	...
	event.create('gun_crate_exotic').food(food => {
		food
    		...
    		.eaten(ctx => {
      		ctx.player.tell("嚼嚼！（咀嚼的声音）美味的木头！嗯！")
          ctx.server.scheduleInTicks(2, ctx.server, function (callback) {
    	        callback.data.runCommand('/ftbquests change_progress ' + ctx.player.name + ' complete 736DFC761A1E0B22');
    			})
      	})
...
onEvent('item.registry', event => {
	event.create('warp_to_death_cookie').food(food => {
		food
          ...
          ctx.server.runCommand('/ftbquests change_progress ' + ctx.player.name + ' complete 2D63E01B226FEB8F');
...
onEvent('item.registry', event => {
	event.create('scary_city_cookie').food(food => {
		food
          ...
          ctx.server.runCommand('/execute at ' + ctx.player.name + ' as @a[distance=..10] run ftbquests change_progress ' + ctx.player.name + ' complete 2D63E01B226FEB8F');
          ctx.server.scheduleInTicks(23, ctx.server, function (callback) {
